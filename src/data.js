...
export class DrawArea extends Sink {
 ...
 export class DrawArea2 extends Sink {
    ...
    addToPath(time,objnum,curclient,x,y,pressure) {
        ...
        var weight=Math.sqrt((this.newx-x)*(this.newx-x)+(this.newy-y)*(this.newy-y))*intw;
    ...
    findPagebreak(pagemin,pagemax)
    {
        ...
        let maxslicepos=Math.round(pagemax/this.numslicesheight);
        let minslicepos=Math.round(pagemin/this.numslicesheight);
        //console.log("findPageBreak",maxslicepos,minslicepos);

        for (let index=maxslicepos;index>=minslicepos; index--) {
          let pagebreak=(index+0.5)*this.numslicesheight;
...
export class Container extends Sink {
    ...
    startPath(time,objnum,curclient,x,y,type,color,w,pressure) {
       var tempbuffer=new ArrayBuffer(36);
    ...
    addToPath(time,objnum,curclient,x,y,pressure) { // todo add objnum
       var tempbuffer=new ArrayBuffer(24);
       var dataview=new DataView(tempbuffer);
    ...
    finishPath(time,objnum,curclient) {
       var tempbuffer=new ArrayBuffer(12);
       var dataview=new DataView(tempbuffer);
   ...
   addPicture(time,objnum,curclient,x,y,width,height,id) {
       ...
       let buflength=id.length/2; // it is hex coded so two bytes per byte
       if (buflength>255) {console.log("id too long!",id,buflength); return; }

       let destlength=32+1+buflength;

       var tempbuffer=new ArrayBuffer(destlength);
       var dataview=new DataView(tempbuffer);
...
export class MemContainer extends Container {
    ...
    replaceStoredData(data)
    {
        ...
        console.log("replaceStorage data bl",data, data.byteLength, this.storageSize,this);
    ...
    redrawElementTo(datasink,pos,lasttime)
    {
        ...
        case 6: {
          //  console.log("addPicture in failsdata redraw");
            let idlength=dataview.getUint8(pos+32);
            let nid="";
            if (length<idlength+1+32) {
                return -1; //damaged data
            }
            for (let i=0;i<idlength;i++ ) {
                let number=dataview.getUint8(pos+32+1+i);
...
export class Collection extends Sink {
    ...
    startPath(time,objnum,curclient,x,y,type,color,w,pressure) {
        ...
        this.containers[storagenum].startPath(time,objnum,curclient,x,y,type,color,w,pressure);

    };

    addToPath(time,objnum,curclient,x,y,pressure) {
       var storagenum=this.lastcontainer[objnum]; // in Normalized coordinates we have rectangular areas
       //console.log("strnm atp",storagenum);
        ...
        this.containers[storagenum].addToPath(time,objnum,curclient,x,y,pressure);
...
export class Dispatcher extends Sink {
    ...
    startPath(time,objnum,curclient,x,y,type,color,w,pressure) {
       ...
       for (i=0;i<this.datasinklist.length;i++) {
           this.datasinklist[i].startPath(timeset,object,client,x,y,type,color,w,pressure);
       }
...
export class DrawObjectGlyph extends DrawObject {
    ...
    {
        var scolor=Color(color).hex();
        
        let penwidth=this.svgscale*width;
        ...
        let px=x*this.svgscale;
        let py=y*this.svgscale;
        ...
        this.area={ left:-2*penw, right:2*penw,
                top: -2*penw,bottom: 2*penw};
    ...
    addToPath(x,y,pressure)
    {
        ...
        let wx=this.lastpoint.x;
        let wy=this.lastpoint.y;
        ...
        this.area={ left: Math.min(px-sx-2*pw,ws.left), right: Math.max(px-sx+2*pw,ws.right),
                        top: Math.min(py-sy-2*pw,ws.top),bottom: Math.max(py-sy+2*pw,ws.bottom)};
        this.pressure=curpress;
        this.version++; // increment version
    }
    ...
    SVGPath() {
        ...
        let glyph=this;
        if (glyph.pathpoints && glyph.pathpoints.length>2) // was 2
        {
            var lastpoint=null;
            if (glyph.pathpoints && glyph.pathpoints.length>2) lastpoint=glyph.pathpoints[glyph.pathpoints.length-1];
            var firstpoint=null;
            ...
            let harr=glyph.pathpoints.length+1
            let pathstrings=new Array(2*harr+1);
            //let lastnx=null;
            //let lastny=null;
            for (let i=0;i<glyph.pathpoints.length;i++)
            {
                let curpoint=glyph.pathpoints[i];
                let dx=0;
                let dy=0;
                if (i>0) {
                ...
                if (norm<curpoint.w*0.5*0.1 ) { // very rare case, but very stupid
                    if (i==0) continue;
                    dx=curpoint.x-glyph.pathpoints[i-1].x;
                    dy=curpoint.y-glyph.pathpoints[i-1].y;
                ...
                let nx=dy*curpoint.w*0.5;
                let ny=-dx*curpoint.w*0.5;
                ...
                if (i==0) {
                    ...
                    wsadd="L"+(curpoint.x-sx+nx).toFixed(2)+","+(curpoint.y-sy+ny).toFixed(2)+" ";
                }
                weadd="L"+(curpoint.x-sx-nx).toFixed(2)+","+(curpoint.y-sy-ny).toFixed(2)+" ";

                pathstrings[i+1]=wsadd;
                pathstrings[2*harr-i-1]=weadd;
                if (i==0) {
                    pathstrings[0]="M"+(curpoint.x-sx-nx).toFixed(2)+","+(curpoint.y-sy-ny).toFixed(2)+" ";
                    pathstrings[1]="A"+(curpoint.w*0.5).toFixed(2)+","+(curpoint.w*0.5).toFixed(2)
                                +",0,1,1,"+(curpoint.x-sx+nx).toFixed(2)+","+(curpoint.y-sy+ny).toFixed(2)+" ";
                }
                if (i==glyph.pathpoints.length-1) {
                    pathstrings[harr]="A"+(curpoint.w*0.5).toFixed(2)+","+(curpoint.w*0.5).toFixed(2)
                                +",0,1,1,"+(curpoint.x-sx-nx).toFixed(2)+","+(curpoint.y-sy-ny).toFixed(2)+" ";
                }
        ...
        } else if (glyph.pathpoints && glyph.pathpoints.length > 0) { // single point case
            var firstpoint=null;
            if (glyph.pathpoints && glyph.pathpoints.length>0) firstpoint=glyph.pathpoints[0];
            let sx=(firstpoint?firstpoint.x:0);
            let sy=(firstpoint?firstpoint.y:0);

            let curpoint = glyph.pathpoints[0];
            // handle single point
            this.svgpathstring = "M" + (curpoint.x - curpoint.w * 0.5-sx).toFixed(2) + "," + (curpoint.y - sy).toFixed(2) + " " +
                "A" + (curpoint.w * 0.5).toFixed(2) + "," + (curpoint.w * 0.5).toFixed(2)
                + ",0,1,1," + (curpoint.x + curpoint.w * 0.5 - sx).toFixed(2) + "," + (curpoint.y - sy).toFixed(2) +
                "A" + (curpoint.w * 0.5).toFixed(2) + "," + (curpoint.w * 0.5).toFixed(2)
                + ",0,1,1," + (curpoint.x - curpoint.w * 0.5 - sx).toFixed(2) + "," + (curpoint.y - sy).toFixed(2) + " Z";
            this.svgpathversion = this.version;
